...
error_log("=== SEND BUSINESS PHONE OTP ENDPOINT HIT ===");
...
try {
    ...
    if ($user['role'] !== 'merchant') {
        ...
        echo json_encode(['status' => 'error', 'message' => 'Only merchants can update business phone numbers']);
        exit;
    }
    ...
    // Check if this phone number is already in use by another store
    $storeModel = new \Model\Store();
    if ($storeModel->storeFieldExists('biz_phone', $formattedPhone, $user['store_id'])) {
        error_log("=== ERROR: PHONE ALREADY EXISTS ===");
        http_response_code(409);
        echo json_encode([
            "status" => "error", 
            "message" => "This phone number is already in use by another business"
        ]);
        exit;
    }
    
    error_log("=== SENDING OTP FOR BUSINESS PHONE UPDATE ===");
    error_log("Phone: " . $formattedPhone);
    error_log("User ID: " . $user['user_id']);
    
    // Send OTP for business phone update
    $otpController = new OtpController();
    $response = $otpController->sendOtp($formattedPhone, 'business_phone_update', null, $user['user_id']);
    
    error_log("=== OTP SEND COMPLETED ===");
    error_log("Response: " . json_encode($response));
    
    echo json_encode($response);
    
} catch (Exception $e) {
    error_log("=== ERROR IN SEND BUSINESS PHONE OTP ===");
