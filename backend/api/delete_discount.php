...
error_log("=== DELETE DISCOUNT ENDPOINT HIT ===");
...
try {
    ...
    if (!isset($data['id']) || !is_numeric($data['id']) || $data['id'] <= 0) {
        error_log("=== ERROR: INVALID DISCOUNT ID ===");
        error_log("Provided ID: " . ($data['id'] ?? 'NOT_SET'));
    ...
    error_log("=== STARTING DISCOUNT DELETION ===");
    error_log("Discount ID to delete: " . $data['id']);
    error_log("User store_id: " . ($user['store_id'] ?? 'NOT_SET'));

    $controller = new DiscountController();
    $response = $controller->deleteDiscount($data['id'], $user);
    
    error_log("=== DISCOUNT DELETION COMPLETED ===");
    error_log("Response: " . json_encode($response));
    
    echo json_encode($response);
    
} catch (Exception $e) {
    error_log("=== ERROR IN DELETE DISCOUNT ===");
