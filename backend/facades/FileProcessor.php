...
class FileProcessor{
    ...
    public function uploadedFiles($key){
        $upls = $_FILES[$key];
        $uplRes = [];
        if(!$upls) return [];
        if(is_array($upls["name"])){
            foreach($upls["name"] as $key => $val){
                $fldata = [
                    "name" => $upls["name"][$key],
                    "tmp_name" => $upls["tmp_name"][$key],
                    "size" => $upls["size"][$key],
                    "type" => $upls["type"][$key],
                    "error" => $upls["error"][$key]
                ];
                array_push($uplRes, $fldata);
            }
        }
        return $uplRes;
    }
