...
public function createSingleItem($data, $user)
{
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
        ...
        error_log("=== ITEM PHOTO UPLOAD DEBUG (CREATE) ===");
        ...
        if (!in_array($fileType, $allowedTypes)) {
            error_log("=== UNSUPPORTED IMAGE FORMAT ERROR (ITEM CREATE) ===");
...
public function updateItem($data, $user)
{
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
        ...
        // Enhanced debugging for image format issues
        error_log("=== ITEM PHOTO UPLOAD DEBUG (UPDATE) ===");
        error_log("File name: " . $fileName);
        error_log("File type (MIME): " . $fileType);
        error_log("File size: " . $fileSize . " bytes");
        error_log("Allowed types: " . implode(', ', $allowedTypes));
        ...
        if (!in_array($fileType, $allowedTypes)) {
            error_log("=== UNSUPPORTED IMAGE FORMAT ERROR (ITEM UPDATE) ===");
            error_log("Received MIME type: " . $fileType);
            error_log("Expected MIME types: " . implode(', ', $allowedTypes));
            error_log("File name: " . $fileName);
